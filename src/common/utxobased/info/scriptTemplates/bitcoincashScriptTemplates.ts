import { OptionalScriptTemplateArgs, ScriptTemplates } from './types'
...
export const BCHScriptTemplates: ScriptTemplates = {
  replayProtection: (pubKey: string): string => {
    return cds(CDS_SIGNATURE, CDS_MESSAGE, CDS_PUBKEY, pubKey).join(
  ...
  checkdatasig: (
    ...
    const cdsSig = optionalArgs?.cdsSig ?? ''
    const cdsMsg = optionalArgs?.cdsMsg ?? ''
    const cdsPubKey = optionalArgs?.cdsPubKey ?? ''
    return cds(cdsSig, cdsMsg, cdsPubKey, pubKey).join(OP_CHECKDATASIG)
  },
  checkdatasigverify: (
    pubKey: string,
    optionalArgs?: OptionalScriptTemplateArgs
    ...
    const cdsMsg = optionalArgs?.cdsMsg ?? ''
    const cdsPubKey = optionalArgs?.cdsPubKey ?? ''
    return cds(cdsSig, cdsMsg, cdsPubKey, pubKey).join(OP_CHECKDATASIGVERIFY)
  }
}
